...
WEATHER_MAIN=$(echo "${WEATHER_INFO}" | grep -o -i -e '\"main\"\:\"[a-z]*\"' | awk -F ':' '{print $2}' | tr -d '"')
WEATHER_TEMP=$(echo "${WEATHER_INFO}" | grep -o -i -e '\"temp\"\:\-\?[0-9]*' | awk -F ':' '{print $2}' | tr -d '"')
